...
def get_script(chr):
	a, _ = get_script_cat(chr)
	return a

def get_category(chr):
	_, a = get_script_cat(chr)
	return a

def get_morph_cluster(morph: tuple, allow_mixed:bool=True, verbose:bool=False):
	hex_morph = binascii.unhexlify("".join(morph))
	try:
		unicode_morph_list = hex_morph.decode('utf-8', errors='strict')
	except UnicodeDecodeError:
		if verbose:
			print(
				f"{hex_morph} couldn.t is not valid byte sequence. Partial decode: {hex_morph.decode('utf-8', errors='backslashreplace')}")
		return None

	# check script of unicode characters
	char_script = [get_script(char) for char in unicode_morph_list if char != 'A']
	if len(char_script) == 0:
		return None

	if len(set(char_script)) > 1:
		# find majority class in list
		if allow_mixed:
			return script2cluster["Mixed"]
		else:
			return script2cluster[max(set(char_script), key=char_script.count)]

	return script2cluster[char_script[0]]


script2cluster = {}
for cluster, scripts in script_clusters.items():
	for script in scripts:
		script2cluster[script] = cluster
