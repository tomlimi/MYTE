...
def get_dataset(tokenizer, task, directory, shrads=10):
	...
	train_loaders = [DataLoader(dataset.map(partial(preprocess_function, tokenizer=tokenizer), batched=True, desc="Running tokenizer"), batch_size=8, shuffle=True) for dataset in train_datasets]
	eval_loader = DataLoader(eval_dataset.map(partial(preprocess_function, tokenizer=tokenizer), batched=True, desc="Running tokenizer"), batch_size=5, shuffle=True)
	test_loader = DataLoader(test_dataset.map(partial(preprocess_function, tokenizer=tokenizer), batched=True, desc="Running tokenizer"), batch_size=5, shuffle=True)

	return train_loaders, eval_loader, test_loader


def train_evaluate(model, train_loaders, eval_loader, lr=1e-4, n_epochs=30, patience=3):
	...
	model, optimizer, eval_loader = accelerator.prepare(model, optimizer, eval_loader)
...
def infer(model, tokenizer, evaluation_loader, model_type, model_size, directory,  split='test'):

	with open(f"{directory}/{split}_{model_type}_{model_size}.csv", 'w', encoding='utf-8') as eval:
		writer = csv.writer(eval, quoting=csv.QUOTE_ALL)
